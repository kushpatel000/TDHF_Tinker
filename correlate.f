...
c      if (exist) then
c         read (string,*,err=70,end=70)  maxgap
c         query = .false.
c      end if
c   70 continue
c      if (query) then
c         write (iout,80)
c   80    format (/,' Maximum Frame Separation to be Used in',
c     &              ' Correlation [ALL] :  ',$)
c         read (input,90)  string
c   90    format (a120)
c         read (string,*,err=100,end=100)  maxgap
c  100    continue
c      end if
c      if (maxgap .eq. 0)  maxgap = last - first
c
