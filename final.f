...
c######################################################     
c     
c     tdhf decallocations and such
c
      if (allocated(tdhffocka))  deallocate(tdhffocka)
      if (allocated(tdhffockb))  deallocate(tdhffockb)
      if (allocated(tdhfeda))    deallocate(tdhfeda)
      if (allocated(tdhfedb))    deallocate(tdhfedb)
      if (allocated(tdhfhc))    deallocate(tdhfhc)
      if (allocated(tdhfgamma)) deallocate(tdhfgamma)
      close(unit=tdhfout)
c
c     CI deallocations and such
c
      if (allocated(ci_gamma))  deallocate(ci_gamma)
      if (allocated(ci_v))      deallocate(ci_v)
      if (allocated(ci_en))     deallocate(ci_en)
      if (allocated(ci_ed))     deallocate(ci_ed)
      if (allocated(ci_hc))     deallocate(ci_hc)
      close(unit=ciout)
c######################################################
